...
- [ ] 001-A1 - Change fail-open dev default to fail-closed production-safe default (Status: Open)
...
- [ ] 016-B2 - Fix require_once for routes to allow re-entry and tests (Status: Open)
- [ ] 017-B3 - Preserve exception chain and types on bootstrap and DB connect (Status: Open)
- [ ] 018-B4 - Fix Rules::required for 0, numbers, and arrays (Status: Open)
- [ ] 019-B5 - Harden requiredArray, minLength, maxLength, requiredIf (Status: Open)
- [ ] 020-B6 - Fix Request stream and has handling for JSON and falsy values (Status: Open)
- [ ] 021-B7 - Make Validator fail loudly on unknown rules and missing fields (Status: Open)
- [ ] 022-B8 - Guard Model::instance and empty table configuration (Status: Open)
...
- [ ] 025-B11 - Replace time() version with content hash (Status: Open)
- [ ] 026-B12 - Make timezone and locale configurable (Status: Open)
- [ ] 027-B13 - Normalize url() slash handling and error path (Status: Open)
...
- [ ] 030-B16 - Harden filesystem helpers against deletion mistakes (Status: Open)
- [ ] 031-B17 - Fix parseBasicTemplate regex injection (Status: Open)
- [ ] 032-B18 - Replace insecure ID generators with random_bytes (Status: Open)
- [ ] 033-B19 - Fix Vite helper HMR edge case (Status: Open)
- [ ] 034-C1 - Reduce static facades with explicit seams and resettable singletons (Status: Open)
- [ ] 035-C2 - Unify environment handling between Config package and app helpers (Status: Open)
...
- [ ] 039-C6 - Validate config shape at bootstrap (Status: Open)
...
- [ ] 042-D1 - Do not force debugMode(false) globally (Status: Open)
...
- [ ] 047-E2 - Version CSS and JS consistently (Status: Open)
- [ ] 048-E3 - Document Vite proxy limits (Status: Open)
- [ ] 049-F1 - Add first-class .env support and stop putting secrets in config.php (Status: Open)
- [ ] 050-F2 - Pin PHP and extensions consistently (Status: Open)
...
- [ ] 055-G1 - Add PHP test harness and first regression tests (Status: Open)
- [ ] 056-G2 - Add PSR-3 logging and use it on all error paths (Status: Open)
- [ ] 057-G3 - Add static analysis and style checks (Status: Open)
- [ ] 058-G4 - Add CI for app code, not just docs FTP (Status: Open)
